...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<?> handleNegocio(NegocioException ex, WebRequest request) {
		...
		ProblemType problemType = ProblemType.VIOLACAO_REGRA_NEGOCIO;
	...
	public ResponseEntity<Object> handleUncaught(Exception ex, WebRequest request) {
		...
		String detail = String.format(MSG_ERRO_GENERICO_USUARIO_FINAL, ex.getMessage());
		Problem problem = createProblemBuilder(status, problemType, detail)
	...
	private ResponseEntity<Object> handleInvalidFormat(InvalidFormatException ex, HttpHeaders headers,
		...
		// pega os atributos que estao com problema na exception
	...
	private ResponseEntity<Object> handlePropertyBinding(PropertyBindingException ex, HttpHeaders headers, 
		...
		String detail = String.format("Atributo '%s' desconhecido.", path);
	...
	private ResponseEntity<Object> handleMethodArgumentTypeMismatch(MethodArgumentTypeMismatchException ex, HttpHeaders headers,
		...
		String detail = String.format("O parametro de URL '%s' recebeu o valor '%s', que é de um tipo inválido. "
					+ "Informe um valor do tipo '%s'.", ex.getName(), ex.getValue(), ex.getRequiredType().getSimpleName());			
		Problem problem = createProblemBuilder(status, problemType, detail)
				.userMessage(detail)
				.build();
		return handleExceptionInternal(ex, problem, new HttpHeaders(), status, request);
	...
	protected ResponseEntity<Object> handleNoHandlerFoundException(NoHandlerFoundException ex, HttpHeaders headers,
			HttpStatus status, WebRequest request) {
		ProblemType problemType = ProblemType.RECURSO_NAO_ENCONTRADO;
		String detail = String.format("O recurso '%s' é inexistente.", ex.getRequestURL());
		Problem problem = createProblemBuilder(status, problemType, detail)
				.userMessage(detail)
				.build();
